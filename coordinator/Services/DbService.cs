...
namespace Karenia.Rurikawa.Coordinator.Services {
    public class DbService {
        ...
        public async Task<Job?> GetJob(FlowSnake id)
            => await db.Jobs.Where(j => j.Id == id).AsNoTracking().SingleOrDefaultAsync();

        public async Task<TestSuite?> GetTestSuite(FlowSnake id)
            => await db.TestSuites.Where(j => j.Id == id).AsNoTracking().SingleOrDefaultAsync();
        ...
        public async Task<List<Announcement>> GetAnnouncements(
            ...
            query = ascending
                ? query.Where(a => a.Id > fromId).OrderBy(a => a.Id)
                : query.Where(a => a.Id < fromId).OrderByDescending(a => a.Id);
            return await query.Take(count).AsNoTracking().ToListAsync();
        }

        public async Task<Announcement?> GetAnnouncement(FlowSnake id)
            => await db.Announcements.Where(a => a.Id == id).AsNoTracking().SingleOrDefaultAsync();
    ...
    public class RedisService {
        public RedisService(string redisConnectionString) => this.redisConnectionString = redisConnectionString;

        private readonly string redisConnectionString;
        private ConnectionMultiplexer? redisConnection;

        public async ValueTask<IDatabase> GetDatabase() => (await GetRedisConnection()).GetDatabase();

        public async ValueTask<ISubscriber> GetSubscriber() => (await GetRedisConnection()).GetSubscriber();
